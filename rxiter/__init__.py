...
def async_share(async_iterable):
    return _SharableAsyncIterable(async_iterable)

def async_replay(async_iterator):
    return _ReplayAsyncIterable(async_iterator)

def async_share_dec(async_iterable_fn):
    def wrapper(*args, **vargs):
        return _SharableAsyncIterable(async_iterable_fn(*args, **vargs))
    return functools.cache(wrapper)

def async_replay_dec(async_iterator_fn):
    def wrapper(*args, **vargs):
        return _ReplayAsyncIterable(async_iterator_fn(*args, **vargs))
    return functools.cache(wrapper)